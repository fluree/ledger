...
(deftest invalid-txns-9
  (let [createPO   {:_id       "purchaseOrder",
                    ...
                    :product   {:_id           "product",
                                ...
                                :quantity      100,
                                :unitOfMeasure "lb"}}
        roastery-opts     {:auth "TfA6vquJMH65oQttpuURWvGnMdPPdAA69PF"
                           :private-key "36abfcd2da19781550d6c9296ada95e11ef0ebfe9acdf3723e59098dc41fe8a5"}
    ...
    (is (= createPOResp "Only a cafe can create a purchaseOrder/id, and it's unchangeable. Value: 124"))

    (is (= createPOResp2 "Only a cafe can create a purchaseOrder/id, and it's unchangeable. Value: 124"))

    (is (= createPOResp3 "Transaction does not adhere to the collection spec: Required predicates: id, product, issuer, issueDate, name."))
...
(deftest valid-txn-9
  (let [txn [{:_id "purchaseOrder",
    ...
    (is (= 5 (-> createShipmentResp :tempids count)))
